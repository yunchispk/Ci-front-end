...
Page({
  ...
  rewrite: function (e) {
    ...
    console.log(sn)
    if (e.currentTarget.dataset.guanlianid == '1') {
      wx.navigateTo({
        url: 'rewrite/rewrite?sn=' + sn,
      })
    } else {
      if (e.currentTarget.dataset.guanlianid == '0') {
        wx.navigateTo({
          url: '/pages/redouble/double?sn=' + sn,
        })
      } else {
        wx.navigateTo({
          url: '/pages/redouble2/double2?sn=' + sn + '?guanlianid=' + e.currentTarget.dataset.guanlianid,
        })
      }
    }
  },
  ...
  delete1: function (e) {
    ...
    const that = this
    wx: wx.request({
      url: 'https://www.ikjmls.cn/user_id/' + app.globalData.openid + '/delete/' + that.data.delete_sn,
      success: function (res) {
        console.log(res)
        that.onShow()
      },
  ...
  onShareAppMessage: function (e) {
    ...
    return {
      ...
      path: '/pages/share/share?iscreating_id=' + e.target.dataset.sn
    }
  }
})