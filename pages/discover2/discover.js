...
Page({
  ...
  onLoad: function (options) {
    ...
    var sn = options.sn
    wx: wx.request({
      url: 'https://www.ikjmls.cn/return_unfinish_id/' + sn,
      success: function (res) {
        that.setData({
        ...
        that.setData({
          ...
          wx.showModal({
            ...
            success: function (res) {
              if (res.confirm) {
                wx.navigateTo({
                  url: '/pages/in_double2/double2?sn=' + sn,
                })
              }
            },
            fail: function (res) { },
  ...
  dianzan: function(e) {
    ...
    console.log(index)
    wx: wx.request({
      url: 'https://www.ikjmls.cn/like/' + index,
      success: function (res) {
        console.log(res)
        for(var i = 0; i<results.length; i++){
          if(results[i].sn == index){
            results[i].guanlianid = 0
            results[i].like_num = res.data.like_num
            break
          }
        }
        console.log(results)
  ...
  onShareAppMessage: function (e) {
    ...
    const that = this
    return {
      title: app.globalData.name + "的发现！",
      desc: app.globalData.name + "发现了什么！快来围观！",
      path: '/pages/share/share?iscreating_id=' + e.target.dataset.index
    }
  }
})